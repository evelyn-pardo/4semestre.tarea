export function keyboardBall(e, pelota, contenedor) {
  ...
  if (e.keyCode === 39) {
    //Fleca Derecha
    if (
      parseInt($pelota.style.left) <
      parseInt(getComputedStyle($contenedor).getPropertyValue("width")) - 80
    ) {
  ...
  if (e.keyCode === 40) {
    //Fleca Abajo
    if (
      parseInt($pelota.style.top) <
      parseInt(getComputedStyle($contenedor).getPropertyValue("height")) - 80
    ) {
      $pelota.style.top = `${parseInt($pelota.style.top) + 20}px`;
    }
  }
